...
def gausseidel(x0, A, b, Tol, niter, modo):
    ...
    en forma matricial: x^{k+1} = T x^{k} + C.

    Retorna:
        tabla_html: tabla de iteraciones en HTML
        s: solución aproximada o "Error"
        aux: True si NO convergió
        sp_radius: radio espectral de la matriz de iteración T
    """
    c = 0
    error = Tol + 1
    E = []          # Lista de errores por iteración
    all_x = []      # Lista de vectores x^{(k)} por iteración
    aux = False     # Flag de fallo

    try:
        D = np.diag(np.diagonal(A))   # Matriz diagonal
        L = -np.tril(A, -1)           # Triangular inferior (con signo -)
        U = -np.triu(A, 1)            # Triangular superior (con signo -)
        ...
        # Radio grande para indicar que no converge
    ...
    while error > Tol and c < niter:
        try:
            ...
            if modo == "cs":
                # Error relativo (cifras significativas)
...
# ==========================================================
#      GENERADOR DE INFORME – GAUSS-SEIDEL
# ==========================================================

def generar_informe_gauss_seidel(
    df,
    A,
    b,
    x0,
    ...
    """
    Genera el informe HTML para el método de Gauss-Seidel
    (igual estilo que los otros métodos del proyecto).
    """
    ...
    if sp_radius is not None:
        if sp_radius < 1:
            texto_radio = (
                ...
                f"ρ(T) ≈ {sp_radius:.6f} (< 1), por lo que el método es convergente "
                "para este sistema."
            )
        else:
            texto_radio = (
                f"El radio espectral de la matriz de iteración es "
                f"ρ(T) ≈ {sp_radius:.6f} (≥ 1), por lo que el método puede no ser "
                "convergente para este sistema."
            )
    else:
        texto_radio = ""
    ...
    if error_final <= tol:
        texto_tol = (
            ...
            "tolerancia, por lo que se considera que el método convergió adecuadamente "
            "para este sistema."
        )
    else:
        texto_tol = (
            "El error no alcanzó la tolerancia objetivo dentro del número máximo de "
            "iteraciones, por lo que se considera que el método no convergió en las "
            "condiciones dadas."
        )
    ...
    informe_html = f"""
    <hr class="mt-5 mb-4">
    <h2>Informe de ejecución – Método de Gauss-Seidel</h2>
    <p>
        Se aplicó el método de Gauss-Seidel al sistema lineal <code>Ax = b</code>, con:
...
def generar_comparacion_errores_gauss_seidel(df, A, b, solucion, tiempo):
    """
    Genera una tabla HTML comparando distintos tipos de error
    para la última iteración del método de Gauss-Seidel.

    df: DataFrame con las iteraciones (columnas x1, x2, ..., Error)
    A, b: sistema Ax = b
    solucion: vector aproximado final
    ...
    # Error absoluto entre iteraciones: ||x^(k) - x^(k-1)||_inf
    ...
    # Norma del residual: ||A x^(k) - b||_inf
    ...
    # Tabla "al estilo" de tus otros métodos:
    data = {
        "Tipo de error": [
